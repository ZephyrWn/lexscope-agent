  ...
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand-block">
        <p class="eyebrow">LexScope Agent</p>
        <h1>法律研判控制台</h1>
        <p class="brand-sub">民商法案例研判与法规检索工作台</p>
      </div>
    ...
    <main class="workspace">
      <header class="workspace-head">
        <div v-if="activeView === 'chat'">
          <p class="workspace-kicker">Active Case Session</p>
        ...
        <div v-else>
          <p class="workspace-kicker">Legal RAG Evaluation</p>
          <h2>{{ selectedEvalDataset?.name || '法规检索评测' }}</h2>
          <p class="workspace-sub">
...
const DEFAULT_SYSTEM_MESSAGE =
  '欢迎使用 LexScope Agent。你可以输入 API Key 获取 JWT，然后围绕民商法案例、合同争议、裁判规则和法规依据发起带研判轨迹的问答。';
...
const DEFAULT_EVAL_DATASET = [
  {
    ...
    category: 'rag_recall',
    chatId: 'eval-rag-a',
    question: '根据知识库，房屋租赁合同纠纷中出租人扩大损失时应承担什么后果？',
    expectedKeywords: ['扩大损失', '赔偿', '租金', '引用'],
  ...
  },
  {
    ...
    chatId: 'eval-rag-b',
    question: '请总结这个 PDF 案例的基本案情、裁判结果和裁判要点。',
    expectedKeywords: ['案情', '裁判', '租赁', '引用'],
  ...
  },
  {
    ...
    chatId: 'eval-rag-b',
    question: '回答时列出引用来源，并提炼可复用的合同审查风险规则。',
    expectedKeywords: ['引用', '合同', '风险', '规则'],
...
const evalDatasetDescription = ref('民商法案例检索、引用覆盖与裁判规则抽取基线集');
